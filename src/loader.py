...
class DataFactory(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2. 전처리 모드 (Train) - 800개 Raw 데이터 로드
        if self.mode == 'train':
            ...
            for v, i in zip(views, indices):
                ...
                # [중요] Selector의 재료가 될 800개 데이터
                node_features_list.append(torch.from_numpy(npz['node_features']).float()) # [800, 256]
                kpts_list.append(torch.from_numpy(npz['kpts']).float())                   # [800, 2]
            
            # [B, 4, 800, C] 형태로 만들기 위해 stack
            data['raw_node_features'] = torch.stack(node_features_list)
            data['raw_kpts'] = torch.stack(kpts_list)

        # 3. 추론/검증 모드 (Val/Test) - 이미지 로드
        else:
            img_paths = [
...
def vo_collate_fn(batch):
    ...
    # 학습 모드 (800개 피처 묶기)
    if 'raw_node_features' in batch[0]:
        result['raw_node_features'] = torch.stack([item['raw_node_features'] for item in batch])
        result['raw_kpts'] = torch.stack([item['raw_kpts'] for item in batch])
